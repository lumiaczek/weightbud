...
<page actionBarHidden={true} class="bg-main">
	<stackLayout>
        ...
        >
            ...
            <label text="Wybierz kategorie" class="text-white text-xl text-center lato mr-4" on:tap={onBack} />
        ...
        <scrollView orientation="vertical" class="w-full mt-2 bg-main h-full">
            <flexBoxLayout flexDirection="column" class="space-y-2 p-4">
                {#each categories as category}
                    <label text={category.name} class="text-2xl text-white text-center p-4 bg-secound" on:tap={() => {gotoCategory(category)}}/>
                {/each}
            </flexBoxLayout>
        </scrollView>
    </stackLayout>
